...
bgafcde dgc adfgcb bceadg fecda bcage ebgcaf gdeca gd gbed | bdeg cdg agecb acbeg`.split(
      "\n"
    );
  ...
  const res = inp
    .flatMap((line) => {
      const [ten, dis] = line
        .split(" | ")
        .map((part) =>
          part.split(" ").map((word) => word.split("").sort().join(""))
        );
      const one: string = ten?.find((word) => word.length === 2) ?? "";
      const four: string = ten?.find((word) => word.length === 4) ?? "";
      const seven: string = ten?.find((word) => word.length === 3) ?? "";
      const eight: string = ten?.find((word) => word.length === 7) ?? "";
      const fifthN = ten?.filter((word) => word.length === 5);
      const sixN = ten?.filter((word) => word.length === 6);
      const oneSp = one?.split("");
      const three: string =
        fifthN?.find((word) => oneSp?.every((c) => word.includes(c))) ?? "";
      const threeSp = three?.split("");
      const leftUp = four?.split("").find((c) => !threeSp?.includes(c)) ?? "";
      const five: string =
        fifthN?.find((word) => word.split("").includes(leftUp)) ?? "";
      const two: string =
        fifthN?.filter((word) => word !== three && word !== five)[0] ?? "";
      const six: string =
        sixN?.find((word) => oneSp?.some((c) => !word.includes(c))) ?? "";
      const leftDown = two?.split("").find((c) => !threeSp?.includes(c)) ?? "";
      const nine: string =
        sixN?.find((word) => !word.split("").includes(leftDown)) ?? "";
      const zero: string =
        sixN?.find((word) => word !== six && word !== nine) ?? "";
      ...
      return +(dis?.map((word) => dict[word]).join("") ?? "");
    })
    .reduce((a, b) => a + b, 0);
